...
def getItemJson(path):
    dict = {}
    itemList = getItemList(path)
    print(type(itemList[0].date))#str
    print(type(itemList[0].size))#int
    for i in range(len(itemList)):
        dict[i] = itemList[i].path + ',' + itemList[i].name + ',' + str(itemList[i].date) + ',' + str(itemList[i].size)
